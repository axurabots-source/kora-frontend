...
export default function Dashboard() {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white mb-1">
            {getTimeGreeting()}, {(profile?.full_name || 'Seller').split(' ')[0]}
          </h1>
      ...
      <div className={`p-8 border rounded-3xl mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 transition-colors ${
        isPositive ? 'bg-green-light/[0.02] border-green-light/20' : 'bg-red-light/[0.02] border-red-light/20'
      }`}>
        <div>
                ...
                <div key={e.id} className="flex items-center justify-between p-4 bg-bg rounded-2xl border border-transparent hover:border-border transition-all hover:translate-x-1 group">
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                      e.entry_type === 'receivable' ? 'bg-green-bg text-green-light' : 'bg-red-bg text-red-light'
                    }`}>
